...
def make_agents_from_yaml(prompts: Dict[str, Any]) -> Dict[str, OllamaLLM]:
    ...
    agents = {}
    ############################ cloudfare Host ############################ 
    # ollama_host = "https://contests-attorneys-eliminate-der.trycloudflare.com"
    ...
    for role, cfg in agents_config.items():
        ...
        ############################ cloudfare Host ############################ 
        # agents[role] = OllamaLLM(model=model_name, base_url=ollama_host, system=system_prompt)
...
if __name__ == "__main__":
    ...
    # Only print selected keys from output
    keys_to_show = ["analysis", "database_summary", "internet_context", "final_summary"]
